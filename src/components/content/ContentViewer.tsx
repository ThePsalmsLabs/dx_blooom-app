...
export function ContentPurchaseCard({
  ...
  const ethBalance = useTokenBalance(effectiveUserAddress, null) // ETH balance
    ...
    try {
      ...
      if (multiPaymentSupported) {
        try {
          ...
          const ethPriceInUsdc = ethPriceQuery.data || BigInt(2000000000) // Default $2000/ETH
          const requiredEthAmount = ethPriceInUsdc > 0 ? 
  ...
  const handleCustomTokenPurchase = useCallback(async () => {
    if (!contentQuery.data || !contractAddresses || !paymentState.customTokenAddress) {
      throw new Error('Custom token payment data not available')
    }

    const selectedToken = paymentState.availableTokens[PaymentMethod.CUSTOM_TOKEN]
    if (!selectedToken) {
      throw new Error('Custom token information not available')
    }

    try {
      // Step 1: Approve token if needed
      if (selectedToken.needsApproval && selectedToken.requiredAmount > BigInt(0)) {
        setPaymentState(prev => ({ ...prev, paymentStep: 'approving' }))
        ...
        await writeContract({
          ...
          args: [contractAddresses.COMMERCE_INTEGRATION, selectedToken.requiredAmount],
        })

        // Wait for approval to complete before proceeding
        // In a production app, you'd wait for the transaction receipt
      ...
      throw new Error(`Custom token purchase failed: ${error instanceof Error ? error.message : 'Transaction failed'}`)
    }
  }, [
    contentQuery.data,
    contractAddresses,
